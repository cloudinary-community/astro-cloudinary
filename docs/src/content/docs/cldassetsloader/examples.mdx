...
Query multiple asset types (images and videos) in a single collection:
...
<CodeBlock>
  ...
  ---
  {assets.map(asset => {
    if (asset.data.resource_type === 'video') {
      return (
        <CldVideoPlayer
          src={asset.data.public_id}
          width={asset.data.width}
...
### Combined: Recursive + Multiple Types

Query all images and videos from a folder tree:
